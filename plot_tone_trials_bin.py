...
spike_path = 'outputECP_lowerthres2_lowlearn_5000_just_tone/spikes.h5'
f = h5py.File(spike_path)
spikes_df = pd.DataFrame(
    {'node_ids': f['spikes']['BLA']['node_ids'], 'timestamps': f['spikes']['BLA']['timestamps']})
...
#plt.hist(spikes)
#plt.show()
plot(node_set=node_set_split,tone_start=5000, spikes_df=spikes_df, ax=axs[0], tone_trial_count= 5, title="baseline NMDA conductance")
plot(node_set=node_set_split,tone_start=27500, spikes_df=spikes_df, ax=axs[1], tone_trial_count= 5, title="50% block")
